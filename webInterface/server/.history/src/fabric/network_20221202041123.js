...
const { FileSystemWallet, Gateway, X509WalletMixin, InMemoryWallet, CouchDBWallet } = require('fabric-network');
...
exports.registerUser = async function(identity, username, password, code){
  ...
  try{
    ...
    const walletPath = path.join(process.cwd(), 'wallet');
    const wallet = new FileSystemWallet(walletPath);
    const wallet1 = new InMemoryWallet(walletPath);
    const wallet2 = new CouchDBWallet(walletPath);
    ...
    const a = await wallet2.exists(appAdmin);
    if(!a){
      console.log(`An !!!! for the admin user ${appAdmin} does not exist in the wallet`);
      let response = {};
      response.error = 'In memory waller';
    }else{
      response.error = 'In memory waller WINNN';
    }
    console.log("$$$$$$$$$$$$$$$$$$$$S")
    if (!adminExists) {
  ...
  //   const secret = await ca.register({ affiliation: '', enrollmentID: code, role: 'client' }, adminIdentity);

  //   const enrollment = await ca.enroll({ enrollmentID: code, enrollmentSecret: secret });
  //   const userIdentity = await X509WalletMixin.createIdentity(orgMSPID, enrollment.certificate, enrollment.key.toBytes());
  //   await wallet.import(code, userIdentity);
  //   console.log(`Successfully registered voter ${username}. Use voterId ${code} to login above.`);
  //   let response = `Successfully registered voter ${username} ${password}. Use voterId ${code} to login above.`;
  //   return response;
  } catch (error) {
    console.error(`Failed to register user + ${username} + : ${code}`);
    let response = {};
    response.error = error;
    return response;
  }
