...
function registerUser(identity, username, password, code){
  ...
  try{
    ...
    const gateway = new Gateway();
     gateway.connect(ccp, { wallet, identity: appAdmin, discovery: gatewayDiscovery });
  ...
  //   // Register the user, enroll the user, and import the new identity into the wallet.
    const secret =  ca.register({ affiliation: '', enrollmentID: username, role: 'client' }, adminIdentity);
    console.log('111111111111111');
    const enrollment =  ca.enroll({ enrollmentID: username, enrollmentSecret: secret });
...
let response = registerUser("ro","1","2","3");
